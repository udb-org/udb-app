...
import { CheckCircleIcon, CircleXIcon, DatabaseIcon, GithubIcon, ListTodo, ListTodoIcon, LoaderIcon, XCircleIcon } from "lucide-react";
...
export default function StatusBar() {
  ...
  return <div className="h-[32px] w-full flex-shrink-0  flex items-center gap-5 text-muted-foreground text-sm">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant={"ghost"} className="gap-1 ">
              {tasks[showTask].status == "success" && <CheckCircleIcon size={12} />}
              {tasks[showTask].status == "running" && <LoaderIcon size={12} />}
          ...
          <PopoverContent  className="space-y-2 w-auto">
            {
              tasks.map((task, index) => {
                return <Button key={index} variant={"ghost"} className="gap-2 wrap-normal  w-auto h-auto">
                  {task.status == "success" && <CheckCircleIcon size={12} />}
                  {task.status == "running" && <LoaderIcon size={12} />}
                  {task.status == "error" && <CircleXIcon size={12} />}
                 <div className="max-w-[300px] text-wrap text-left">
