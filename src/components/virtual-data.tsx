...
import { ArrowDownAZIcon, ArrowDownNarrowWideIcon, ArrowUpDownIcon, ArrowUpNarrowWideIcon, ArrowUpZAIcon, ChartBarIcon, ChartPieIcon, DownloadIcon, FunnelPlus, RecycleIcon, SaveIcon } from "lucide-react";
...
export function VirtualData(props: {
    source: any;
    height?: number;
}) {
    ...
    //可视列
    const [startColumnIndex, endColumnIndex, colTotalWidth] = React.useMemo(() => {
        ...
        for (let i = startColumnIndex; i < columnCount; i++) {
            ...
            if (columnLefts[i] + columnWidths[i] > columnLefts[startColumnIndex] + containerWidth.current) {
                endColumnIndex = i;
        ...
        console.log("可视列", startColumnIndex, endColumnIndex + 1, colTotalWidth);
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (event.metaKey && event.key === 'c') {
                console.log("Ctrl+C", selectSvgRef.current && selectSvgRef.current.style.display);
                //复制
                if (selectSvgRef.current && selectSvgRef.current.style.display === "block" && selectRange) {
                    //复制选中的内容
                    let text = "";
    ...
    //排序
    const [sortType, setSortType] = React.useState<"asc" | "desc" | "null">("null");
    //筛选
    ...
    const [filterValueCounts, setFilterValueCounts] = React.useState<Map<string, number>>(new Map());
    //选择项
    const [filterSelected, setFilterSelected] = React.useState<string[]>([]);

    return <div className="w-full h-full flex flex-col text-sm" style={{
        height: props.height
    }}>
        <div className="h-[26px] w-full flex items-center flex-shrink-0 gap-1">
            {
                //工具栏
            }
            ...
            <Button variant={"ghost"} size={"sm"} className="w-6 h-6 p-[5px]"
                onClick={() => {
                    //更具选中的第一列进行排序
                    if (selectRange) {
                        ...
                        if (column) {
                            ...
                            if (_sortType === "null") {
                                ...
                                const sortedData = data.sort((a, b) => {
                                    ...
                                    //数字类型
                                    const numberTypes = ["DOUBLE", "INTEGER", "DECIMAL", "FLOAT", "NUMERIC", "INT", "BIGINT", "SMALLINT", "TINYINT"];
                ...
                <TooltipProvider>
                    <Tooltip>
                        <TooltipTrigger asChild><>
                            {sortType === "null" && <ArrowUpDownIcon />}
                            {sortType === "asc" && <ArrowDownAZIcon />}
                            {sortType === "desc" && <ArrowUpZAIcon />}
                        </>
                        ...
                        onClick={() => {
                            ...
                            if (selectRange) {
                                ...
                                if (column) {
                                    ...
                                    //统计该列的所有值及其数量,并去重
                                    const _values = props.source.data.map(item => item[columnName]);
                                    ...
                                    const _values2 = _values.filter((item, index, self) => self.indexOf(item) === index);
                                    //设置筛选的值
                ...
                <PopoverContent>
                    <div className="flex">
                        <div className="flex-1">
                            Filter By <span className="text-primary font-bold">{filterColumn && filterColumn.columnLable}</span>
                        </div>

                    </div>
                    <Separator />
                    <div className="space-y-1 mt-1">
                        <Input placeholder="filter" className="shadow-none" value={filterValue} onChange={(e) => {
                            const val = e.target.value;
                            setFilterValue(val);
                            //过滤
                            const _values = filterValues.filter(item => item.includes(val));
                            setFilterShowValues(_values);
                        }} />
                    </div>
                    <div className="space-y-1 mt-1 whitespace-nowrap">
                        <Button variant={"ghost"} size={"sm"} className=" h-6 p-[5px] text-sm"
                            onClick={() => {
                                //选择所有
                                setFilterSelected(filterShowValues);
                            }}
                        >
                            All <span className="text-muted-foreground">
                        ...
                        <Button variant={"ghost"} size={"sm"} className=" h-6 p-[5px] text-sm"
                            onClick={() => {
                                //反选
                                const selected = filterShowValues.filter(item => !filterSelected.includes(item));
                        ...
                        <Button variant={"ghost"} size={"sm"} className="h-6 p-[5px] text-sm"
                            onClick={() => {
                                //选择重复项
                                const selected = filterShowValues.filter(item => filterValueCounts.get(item) > 1);
                                setFilterSelected(selected);
                            }}
                        ...
                        <Button variant={"ghost"} size={"sm"} className=" h-6 p-[5px] text-sm" onClick={() => {
                            //清空
                            setFilterSelected([]);
                        }}>
                            Clear
                    ...
                    <div className="h-[300px]">
                        <VirtualList items={filterShowValues} estimateHeight={26} renderItem={
                            (item: any, index: number) => {
                                return <div key={index} className="flex items-center h-[26px] gap-2">
                                    ...
                                    <Checkbox className="shadow-none"
                                        ...
                                        onCheckedChange={(checked) => {
                                            if (checked) {
                                                ...
                                                setFilterSelected(filterSelected.filter(i => i !== item));
                                            }
                                        }}
                                    />
                    ...
                    <div className="mt-1 flex items-center">
                        ...
                        <div>
                            <Button size={"sm"} className=" h-6 p-[5px] text-sm"
                                onClick={() => {
                                    //按照选中的项进行筛选
                                    if (filterSelected.length > 0) {
                                        const filteredData = props.source.data.filter((item: any) => filterSelected.includes(item[filterColumn.columnName]));
            ...
            <Button variant={"ghost"} size={"sm"} className="w-6 h-6 p-[5px]"
                onClick={() => {
                    ...
                    const changedCells: { row: number; col: number; oldValue: any; newValue: any }[] = [];
                    for (let row = 0; row < data.length; row++) {
                        for (let col = 0; col < columns.length; col++) {
                            const columnName = columns[col].columnLable;
                            if (props.source[row] && props.source[row][columnName] !== data[row][columnName]) {
                                changedCells.push({
                                    row,
                                    col,
        ...
        <div className="h-[26px] flex-shrink-0 w-full  flex items-center overflow-auto border-b-1 "
            style={{
                scrollbarWidth: 'none'
            }}
        >
            ...
            <div className="relative h-full" style={{
            ...
            }}>
                {
                    columns.slice(startColumnIndex, endColumnIndex).map((col, index) =>
                        <div key={index} className={cn("flex-1 h-full text-center absolute ")}
                            style={{
                        ...
                        >
                            <div className="flex items-center h-full justify-center">
                                <div className="h-full w-[1px] flex-shrink-0" onMouseDown={(e) => {
                                }}>
                                </div>
                                ...
                                <div className="h-full w-[1px] flex-shrink-0 bg-background cursor-col-resize relative right-[0px] z-10" onMouseDown={(e) => {
                                    e.stopPropagation();
                                    e.preventDefault();
                                    if (selectSvgRef.current) {
                                        selectSvgRef.current.style.display = "none";
                                    ...
                                    const move = (e: MouseEvent) => {
                                        ...
                                        if (colLineRef.current) {
                                            ...
                                            colLineRef.current.style.left = (x - startX) + columnLefts[index + startColumnIndex] + columnWidths[index + startColumnIndex] + "px";
                                        }
                                    };
                                    const up = (e: MouseEvent) => {
                                        if (colLineRef.current) {
                                        ...
                                        setColumnLefts(columnLefts.map((item, i) => {
                                            if (i > index + startColumnIndex) {
                                                return item + width;
                                            }
                                            return item;
                                        }));
                                        setColumnWidths(columnWidths.map((item, i) => {
                                            if (i == index + startColumnIndex) {
                                                return item + width;
                                            }
                                            return item;
        ...
        <div className="flex flex-1 h-full overflow-hidden">
            <div className="overflow-auto  h-full relative flex-shrink-0" style={{
                width: rowNoWidth,
                scrollbarWidth: 'none'
            }}>
                {
                ...
                >
                    {Array(endRowIndex - startRowIndex).fill(0).map((item, index) =>
                        <div key={index} className={cn("absolute border-b-1 border-r-1")}
                            style={{
                            ...
                            onMouseDown={(e) => {
                                ...
                                const fY: any = containerRef.current?.getBoundingClientRect().y;
                                const lY = startY - fY;
                        ...
                        >
                            <div className="flex h-full flex-col ">
                                ...
                                <div className="flex-1 flex items-center justify-end pr-1" >{index + startRowIndex}</div>
                                <div className="w-full h-[1px] bg-background cursor-row-resize relative bottom-[-1px] z-10"
                                    onMouseDown={(e) => {
                                        e.stopPropagation();
                                        e.preventDefault();
                                        if (selectSvgRef.current) {
                                        ...
                                        const move = (e: MouseEvent) => {
                                            ...
                                            if (rowLineRef.current) {
                                                ...
                                                rowLineRef.current.style.top = (y - startY) + rowTops[index + startRowIndex] + rowHeights[index + startRowIndex] + "px";
                                            }
                                        };
                                        const up = (e: MouseEvent) => {
                                            if (rowLineRef.current) {
                                            ...
                                            setRowTops(rowTops.map((item, i) => {
                                                if (i > index + startRowIndex) {
                                                    return item + height;
                                                }
                                                return item;
                                            }));
                                            setRowHeights(rowHeights.map((item, i) => {
                                                if (i == index + startRowIndex) {
                                                    return item + height;
                                                }
                                                return item;
            ...
            <div className="flex-1 h-full overflow-auto  relative " ref={containerRef}
                style={{
                    scrollbarWidth: 'none'
                }}
                onWheel={handleWheel}
            >
                ...
                >
                    ...
                    }
                    {
                        data.slice(startRowIndex, endRowIndex).map((row, rowIndex) => <>
                            {columns.slice(startColumnIndex, endColumnIndex).map((col, columnIndex) =>
                                <div key={columnIndex} className={cn("absolute")}
                                    style={{
                                    ...
                                    onDoubleClick={(e) => {
                                        ...
                                        if (textareaRef.current) {
                                            ...
                                            textareaRef.current.style.top = rowTops[rowIndex + startRowIndex] + "px";
                                            textareaRef.current.style.left = columnLefts[columnIndex + startColumnIndex] + "px";
                                            textareaRef.current.style.height = rowHeights[rowIndex + startRowIndex] + "px";
                                            textareaRef.current.style.width = columnWidths[columnIndex + startColumnIndex] + "px";
                                            //修改后
                    ...
                    <div ref={colLineRef} className="absolute top-0 bottom-0 w-[1px] border-primary border-dashed border-r-[1px] hidden" ></div>
                    <div ref={rowLineRef} className="absolute left-0 right-0 h-[1px] border-primary border-dashed  border-b-[1px]  hidden" ></div>
                    <textarea ref={textareaRef} className="absolute top-0 left-0 resize-none  bg-card hidden"  ></textarea>
                </div>
                <div
                ...
                >
                    ...
                    }
                    {
                        data.slice(startRowIndex, endRowIndex).map((row, rowIndex) => <>
                            {columns.slice(startColumnIndex, endColumnIndex).map((col, columnIndex) =>
                                <div key={columnIndex} className={cn("absolute border-r-1 border-b-1")}
                                    style={{
                                        top: rowTops[rowIndex + startRowIndex],
                                        left: columnLefts[columnIndex + startColumnIndex],
                                        width: columnWidths[columnIndex + startColumnIndex],
                    ...
                    <svg ref={selectSvgRef} className="absolute bg-primary/10 hidden z-0 size-0"></svg>
                </div>
            </div>
        </div>
